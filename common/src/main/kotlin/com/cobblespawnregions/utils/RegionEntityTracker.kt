...
object RegionEntityTracker {
    ...
    fun markChunkUnloading(world: ServerLevel, chunkPos: ChunkPos) {
        unloadingChunks.add(chunkKey(world.dimension().location().toString(), chunkPos))
    }

    fun markChunkLoaded(world: ServerLevel, chunkPos: ChunkPos) {
        unloadingChunks.remove(chunkKey(world.dimension().location().toString(), chunkPos))
    }

    fun isChunkUnloading(entity: PokemonEntity): Boolean =
        chunkKey(entity.level().dimension().location().toString(), entity.chunkPosition()) in unloadingChunks

    fun reconcileLoadedChunk(world: ServerLevel, regionId: String, chunkPos: ChunkPos) {
        val dimension = world.dimension().location().toString()
        val liveSpawnIds = world.getEntitiesOfClass(PokemonEntity::class.java, chunkBox(world, chunkPos)) { entity ->
            val data = entity.pokemon.persistentData
    ...
    fun loadedManagedEntities(world: ServerLevel, regionId: String, regionBox: AABB? = null): List<PokemonEntity> {
        val dimension = world.dimension().location().toString()
        ...
        if (regionBox != null) {
            world.getEntitiesOfClass(PokemonEntity::class.java, regionBox) { entity ->
    ...
    fun rebuildFromWorld(world: ServerLevel, regionId: String, regionBox: AABB) {
        world.getEntitiesOfClass(PokemonEntity::class.java, regionBox) { entity ->
            entity.pokemon.persistentData.getString(REGION_KEY) == regionId
        }.forEach { entity ->
            trackLoadedEntity(entity)
    ...
    private fun chunkBox(world: ServerLevel, chunkPos: ChunkPos): AABB = AABB(
        chunkPos.minBlockX.toDouble(),
        world.minBuildHeight.toDouble(),
        chunkPos.minBlockZ.toDouble(),
        chunkPos.maxBlockX.toDouble() + 1.0,
        world.maxBuildHeight.toDouble(),
        chunkPos.maxBlockZ.toDouble() + 1.0
    )
