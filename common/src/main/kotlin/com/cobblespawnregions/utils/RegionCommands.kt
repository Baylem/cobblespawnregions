...
object RegionCommands {
    ...
    fun register() {
        manager.command("csr", permission = permission("csr")) {
            subcommand("giveclaimstick", permission = permission("giveclaimstick")) {
                then(
                    RequiredArgumentBuilder.argument<CommandSourceStack, String>("mode", StringArgumentType.word())
                        .suggests { _, builder ->
            ...
            subcommand("reload", permission = permission("reload")) {
                executes { ctx ->
                    ...
                    ctx.source.sendSystemMessage(Component.literal("§a[CSR] §fConfig reloaded."))
                    1
                }
            }
            ...
            subcommand("migratecobblespawners", permission = permission("migratecobblespawners")) {
                executes { ctx ->
                    ...
                    ctx.source.sendSystemMessage(Component.literal("$color[CSR] §f${result.message}"))
    ...
    private fun CommandManager.CommandConfig.registerFlatRegionCommands() {
        ...
        subcommand("killspawned", permission = permission("killspawned")) {
            ...
            }.then(RequiredArgumentBuilder.argument<CommandSourceStack, String>("mode", StringArgumentType.word())
                .suggests { _, builder ->
        ...
        subcommand("rename", permission = permission("rename")) {
            then(regionIdArg()
                .then(RequiredArgumentBuilder.argument<CommandSourceStack, String>("newName", StringArgumentType.word())
                    .executes { ctx ->
        ...
        subcommand("forcespawn", permission = permission("forcespawn")) {
            ...
            }.then(RequiredArgumentBuilder.argument<CommandSourceStack, Int>("amount", IntegerArgumentType.integer(1, 100))
                .executes { ctx ->
                    forceSpawn(
                        ctx.source,
                        StringArgumentType.getString(ctx, "regionId"),
        ...
        subcommand("playeractivation", permission = permission("playeractivation")) {
            then(regionIdArg()
                .then(RequiredArgumentBuilder.argument<CommandSourceStack, String>("enabled", StringArgumentType.word())
                    .suggests { _, builder ->
                        builder.suggest("true")
                        builder.suggest("false")
                        builder.buildFuture()
                    }
                    .then(RequiredArgumentBuilder.argument<CommandSourceStack, Int>("range", IntegerArgumentType.integer(0, 512))
                        .executes { ctx ->
                            setPlayerActivation(
                                ctx.source,
                                StringArgumentType.getString(ctx, "regionId"),
        ...
        subcommand("priority", permission = permission("priority")) {
            then(regionIdArg()
                .then(RequiredArgumentBuilder.argument<CommandSourceStack, Int>("value", IntegerArgumentType.integer(-1000, 1000))
                    .executes { ctx ->
                        setRegionPriority(
                            ctx.source,
                            StringArgumentType.getString(ctx, "regionId"),
    ...
    private fun openRegionList(source: CommandSourceStack): Int {
        val player = playerOrError(source) ?: return 0
        RegionListGui.open(player)
        return 1
    }

    private fun listRegions(source: CommandSourceStack): Int {
        val regions = RegionsConfig.regions
        if (regions.isEmpty()) {
            source.sendSystemMessage(Component.literal("§a[CSR] §fNo regions defined yet."))
            return 1
        }

        source.sendSystemMessage(Component.literal("§a[CSR] §f${regions.size} region(s):"))
    ...
    private fun deleteRegion(source: CommandSourceStack, regionId: String): Int {
        if (!RegionsConfig.removeRegion(regionId)) {
            source.sendFailure(Component.literal("No region found with id '$regionId'."))
            return 0
        }
        ...
        source.sendSystemMessage(Component.literal("§a[CSR] §fDeleted region §e$regionId§f."))
        return 1
    }

    private fun toggleVisualization(player: ServerPlayer, source: CommandSourceStack, regionId: String): Int {
        ...
        if (visualizations.remove(regionId)) {
            ...
            player.displayClientMessage(Component.literal("§a[CSR] §fStopped visualizing §e${region.regionName}§f."), false)
        } else {
    ...
    private fun toggleAllVisualizations(player: ServerPlayer, source: CommandSourceStack): Int {
        val regionIds = RegionsConfig.regions.keys.toSet()
        if (regionIds.isEmpty()) {
            source.sendFailure(Component.literal("No regions are configured."))
            return 0
        }
        ...
        if (allVisible) {
            ...
            player.displayClientMessage(Component.literal("§a[CSR] §fStopped visualizing §eall regions§f."), false)
        } else {
            active.addAll(regionIds)
            CobbleSpawnRegions.requestParticleUpdate(player, "started visualizing all regions", logRequest = true)
            player.displayClientMessage(Component.literal("§a[CSR] §fVisualizing §eall ${regionIds.size} regions§f."), false)
        }
        return 1
    }

    private fun openRegionEditor(player: ServerPlayer, source: CommandSourceStack, regionId: String): Int {
        val region = RegionsConfig.getRegion(regionId) ?: run {
            source.sendFailure(Component.literal("No region found with id '$regionId'."))
            return 0
        }
        RegionEditorGui.open(player, regionId)
        player.displayClientMessage(Component.literal("§a[CSR] §fOpened editor for §e${region.regionName}§f."), false)
        return 1
    }

    private fun teleportToRegion(player: ServerPlayer, source: CommandSourceStack, regionId: String): Int {
        val region = RegionsConfig.getRegion(regionId) ?: run {
            source.sendFailure(Component.literal("No region found with id '$regionId'."))
            return 0
        }
        ...
        val surfaceY = world.getHeight(Heightmap.Types.MOTION_BLOCKING, x.toInt(), z.toInt())
        val y = if (surfaceY in minY..maxY) surfaceY.toDouble() else (minY + maxY) / 2.0 + 0.5

        player.teleportTo(world, x, y, z, player.yRot, player.xRot)
        player.displayClientMessage(Component.literal(
    ...
    private fun killSpawned(source: CommandSourceStack, regionId: String, mode: String = "loaded"): Int {
        if (regionId.equals("all", ignoreCase = true)) {
    ...
    private fun killSpawnedInAllRegions(source: CommandSourceStack, mode: String): Int {
    ...
    private fun killSpawnedInRegion(source: CommandSourceStack, regionId: String, mode: String = "loaded"): Int {
        val region = RegionsConfig.getRegion(regionId) ?: run {
            source.sendFailure(Component.literal("No region found with id '$regionId'."))
            return 0
        }
    ...
    private fun normalizedKillMode(source: CommandSourceStack, mode: String): String? {
        val normalized = mode.lowercase()
        if (normalized in setOf("loaded", "tracked", "all")) return normalized
        source.sendFailure(Component.literal("Unknown kill mode '$mode'. Use: loaded or tracked."))
        return null
    }
    ...
    private fun addPokemon(
        ...
        val entry = try {
            ...
            source.sendFailure(Component.literal(e.message ?: "Unknown Pokemon '$pokemonName'."))
            return 0
        }

        if (!RegionsConfig.addPokemonToRegion(regionId, entry)) {
            source.sendFailure(Component.literal("That Pokemon entry already exists in '${region.regionName}'."))
            return 0
        }
    ...
    private fun removePokemon(
        ...
        if (!RegionsConfig.removePokemonFromRegion(regionId, pokemonName, formName, aspects)) {
            source.sendFailure(Component.literal("No matching Pokemon entry found in '${region.regionName}'."))
            return 0
        }
        source.sendSystemMessage(Component.literal(
    ...
    private fun renameRegion(source: CommandSourceStack, regionId: String, newName: String): Int {
        val region = RegionsConfig.renameRegionDisplay(regionId, newName) ?: run {
            source.sendFailure(Component.literal("No region found with id '$regionId'."))
            return 0
        }
        source.sendSystemMessage(Component.literal("§a[CSR] §fRenamed region §e${region.regionId} §fto §e${region.regionName}§f."))
        return 1
    }

    private fun inspectNearest(source: CommandSourceStack): Int {
        val player = playerOrError(source) ?: return 0
        val range = 32.0
        val box = AABB(
        ...
        val nearest = player.serverLevel().getEntitiesOfClass(PokemonEntity::class.java, box) { true }
            .minByOrNull { it.distanceToSqr(player) } ?: run {
                source.sendFailure(Component.literal("No Pokemon found within ${range.toInt()} blocks."))
                return 0
            }
        ...
        source.sendSystemMessage(Component.literal("§a[CSR] §fNearest Pokemon: §e${pokemon.species.name} §7lv ${pokemon.level}"))
        source.sendSystemMessage(Component.literal("§7UUID: §f${nearest.uuid}"))
        source.sendSystemMessage(Component.literal("§7Form: §f${pokemon.form.name} §7Aspects: §f${pokemon.aspects.joinToString(", ").ifEmpty { "none" }}"))
        source.sendSystemMessage(Component.literal("§7Managed Region: §f${data.getString(RegionEntityTracker.REGION_KEY).ifEmpty { "none" }}"))
        source.sendSystemMessage(Component.literal("§7Exclusion Conditions (${conditions.size}):"))
        source.sendSystemMessage(Component.literal("§8Complete list also written to the server console."))
        conditions.chunked(10).forEach { batch ->
            source.sendSystemMessage(Component.literal("§f${batch.joinToString(", ")}"))
        }
        return 1
    }

    private fun forceSpawn(source: CommandSourceStack, regionId: String, amount: Int): Int {
        val region = RegionsConfig.getRegion(regionId) ?: run {
            source.sendFailure(Component.literal("No region found with id '$regionId'."))
            return 0
        }
    ...
    private fun rescanSpawnPoints(source: CommandSourceStack, regionId: String?): Int {
    ...
    private fun rescanSpawnPointsForRegion(regionId: String, region: RegionData, source: CommandSourceStack): Int {
        SpawnPointStore.clearRegion(regionId)
        RegionSpawnHelper.resetSpawnTimer(regionId)
        return SpawnPointScanner.scanLoadedChunks(regionId, region, source.server)
    }

    private fun showRegionInfo(source: CommandSourceStack, regionId: String): Int {
        val region = RegionsConfig.getRegion(regionId) ?: run {
            source.sendFailure(Component.literal("No region found with id '$regionId'."))
            return 0
        }
        source.sendSystemMessage(Component.literal("§a[CSR] §e${region.regionName} §8(${region.regionId})"))
        source.sendSystemMessage(Component.literal("§7Dimension: §f${region.dimension} §7Mode: §f${region.mode} §7Priority: §f${region.priority}"))
        source.sendSystemMessage(Component.literal("§7Bounds: §f(${region.pos1.x},${region.pos1.y},${region.pos1.z}) -> (${region.pos2.x},${region.pos2.y},${region.pos2.z})"))
        source.sendSystemMessage(Component.literal("§7Custom Pokemon: §f${region.selectedPokemon.size} §7Timer: §f${region.spawnTimerTicks} §7Amount: §f${region.spawnAmountPerSpawn}"))
        source.sendSystemMessage(Component.literal("§7Max Alive: §f${region.maxTotalSpawns} §7Tracked: §f${RegionEntityTracker.countTotal(regionId)}"))
        source.sendSystemMessage(Component.literal("§7Require Player: ${flag(region.requirePlayerInRange)} §7Range: §f${region.playerActivationRange.toInt()}"))
        source.sendSystemMessage(Component.literal("§7Natural Disable All: ${flag(region.spawnRestrictions.disableAll)} §7Blocked Species: §f${region.spawnRestrictions.disallowedSpecies.size}"))
        return 1
    }

    private fun cloneRegionFromSelection(player: ServerPlayer, source: CommandSourceStack, sourceRegionId: String, newName: String): Int {
        val sourceRegion = RegionsConfig.getRegion(sourceRegionId) ?: run {
            source.sendFailure(Component.literal("No region found with id '$sourceRegionId'."))
            return 0
        }
        val regionId = sanitize(newName)
        if (RegionsConfig.getRegion(regionId) != null) {
            source.sendFailure(Component.literal("Region '$regionId' already exists."))
            return 0
        }
        ...
        source.sendSystemMessage(Component.literal("§a[CSR] §fCloned §e${sourceRegion.regionName} §fto §e${cloned.regionName}§f."))
        return 1
    }

    private fun reclaimRegionFromSelection(player: ServerPlayer, source: CommandSourceStack, regionId: String): Int {
    ...
    private fun setSpawnAmount(source: CommandSourceStack, regionId: String, amount: Int): Int {
        val region = RegionsConfig.updateRegion(regionId) { it.spawnAmountPerSpawn = amount } ?: run {
            source.sendFailure(Component.literal("No region found with id '$regionId'."))
            return 0
        }
        source.sendSystemMessage(Component.literal("§a[CSR] §fSet §e${region.regionName} §fspawn amount to §e${region.spawnAmountPerSpawn}§f."))
        return 1
    }

    private fun setPlayerActivation(source: CommandSourceStack, regionId: String, enabledRaw: String, range: Int?): Int {
        val enabled = parseBoolean(enabledRaw) ?: run {
            source.sendFailure(Component.literal("Use true or false for enabled."))
            return 0
        }
        val region = RegionsConfig.updateRegion(regionId) {
    ...
    private fun setRegionPriority(source: CommandSourceStack, regionId: String, value: Int): Int {
    ...
    private fun checkCurrentRegion(source: CommandSourceStack): Int {
        val player = playerOrError(source) ?: return 0
        ...
        if (region == null) {
            player.displayClientMessage(Component.literal("§a[CSR] §fYou are §cnot §finside any region."), false)
            return 1
        }

        val restr = region.spawnRestrictions
        player.displayClientMessage(Component.literal("§a[CSR] §fControlling region: §e${region.regionName} §8(${region.regionId}) §7priority §f${region.priority}"), false)
        if (matches.size > 1) {
            player.displayClientMessage(Component.literal("§a[CSR] §fOverlaps here: §7" +
                    matches.drop(1).joinToString(", ") { "${it.regionName}(${it.priority})" }), false)
        }
        player.displayClientMessage(Component.literal("§a[CSR] §fActive restrictions:"), false)
        player.displayClientMessage(Component.literal("§7  disableAll: ${flag(restr.disableAll)}"), false)
        player.displayClientMessage(Component.literal("§7  excludeOwnedPokemon: ${flag(restr.excludeOwnedPokemon)}"), false)
        player.displayClientMessage(Component.literal("§7  blockedSpecies (${restr.disallowedSpecies.size}): §f${restr.disallowedSpecies.joinToString(", ").ifEmpty { "none" }}"), false)
        return 1
    }

    private fun worldForRegion(source: CommandSourceStack, region: RegionData): ServerLevel? {
        val id = ResourceLocation.tryParse(region.dimension) ?: run {
            source.sendFailure(Component.literal("Invalid dimension '${region.dimension}' for region '${region.regionId}'."))
            return null
        }
        val key = ResourceKey.create(Registries.DIMENSION, id)
        return source.server.getLevel(key) ?: run {
            source.sendFailure(Component.literal("Dimension '${region.dimension}' is not loaded."))
            null
        }
    }

    private fun giveStick(player: ServerPlayer, source: CommandSourceStack, mode: StickMode): Int {
        val configuredId = RegionsConfig.config.claimItem
        val itemId = ResourceLocation.tryParse(configuredId)
        val claimItem = if (itemId != null && BuiltInRegistries.ITEM.containsKey(itemId)) {
        ...
        stick.set(DataComponents.CUSTOM_NAME, Component.literal(label))

        val nbt = CompoundTag()
        nbt.putBoolean("cobblespawnregions:is_region_stick", true)
        nbt.putString("cobblespawnregions:mode", mode.name)
        stick.set(DataComponents.CUSTOM_DATA, CustomData.of(nbt))

        if (!player.inventory.add(stick)) player.drop(stick, false)

        source.sendSuccess(
            { Component.literal("§aGave §f$plainName §ato §f${player.name.string}§a.") },
            true
        )
        return 1
    }

    private fun createRegion(player: ServerPlayer, source: CommandSourceStack, name: String): Int {
        ...
        if (RegionsConfig.getRegion(regionId) != null) {
            player.displayClientMessage(Component.literal("§c[CSR] §fRegion §e$regionId §falready exists."), false)
            return 0
        }
        ...
        val (p1, p2) = when (sel.mode) {
            StickMode.CHUNK -> {
                ...
                SerializableBlockPos(minOf(c1.minBlockX, c2.minBlockX), world.minBuildHeight, minOf(c1.minBlockZ, c2.minBlockZ)) to
                        SerializableBlockPos(maxOf(c1.maxBlockX, c2.maxBlockX), world.maxBuildHeight, maxOf(c1.maxBlockZ, c2.maxBlockZ))
            }
    ...
    private fun pokemonCommandArgs(
        handler: (CommandSourceStack, String, String, String?, Set<String>) -> Int
    ): RequiredArgumentBuilder<CommandSourceStack, String> {
        val region = regionIdArg()
        val pokemon = RequiredArgumentBuilder.argument<CommandSourceStack, String>("pokemon", StringArgumentType.word())
            .executes { ctx ->
                handler(
                    ctx.source,
                    StringArgumentType.getString(ctx, "regionId"),
            ...
            .then(RequiredArgumentBuilder.argument<CommandSourceStack, String>("form", StringArgumentType.word())
                .executes { ctx ->
                    handler(
                        ctx.source,
                        StringArgumentType.getString(ctx, "regionId"),
                ...
                .then(RequiredArgumentBuilder.argument<CommandSourceStack, String>("aspects", StringArgumentType.greedyString())
                    .executes { ctx ->
                        handler(
                            ctx.source,
                            StringArgumentType.getString(ctx, "regionId"),
    ...
    private fun regionIdArg() =
        RequiredArgumentBuilder.argument<CommandSourceStack, String>("regionId", StringArgumentType.word())
            .suggests { _, builder ->
    ...
    private fun parseStickMode(raw: String, source: CommandSourceStack): StickMode? =
        when (raw.lowercase()) {
            "coords" -> StickMode.COORDS
            "chunk" -> StickMode.CHUNK
            else -> {
                source.sendFailure(Component.literal("Unknown mode '$raw'. Use: coords, chunk."))
                null
            }
        }
    ...
    private fun playerOrError(source: CommandSourceStack): ServerPlayer? {
        val player = source.player
        if (player == null) source.sendFailure(Component.literal("Only players can use this command."))
        return player
    }
}
