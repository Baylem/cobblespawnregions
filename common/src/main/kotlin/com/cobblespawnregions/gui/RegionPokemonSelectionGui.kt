...
object RegionPokemonSelectionGui {
    ...
    private val playerComputations = ConcurrentHashMap<ServerPlayer, CompletableFuture<Void>>()
    ...
    fun open(player: ServerPlayer, regionId: String, page: Int = 0) {
        invalidateCache()
    ...
    private fun handleSortClick(ctx: InteractionContext, player: ServerPlayer, regionId: String) {
        when (ctx.clickType) {
            ClickAction.PRIMARY -> {
                ...
                refresh(player, regionId)
                player.displayClientMessage(Component.literal("§7[CSR] Sort: ${sortMethod.name}"), false)
            }
            ClickAction.SECONDARY -> RegionPokemonSearchGui.open(player, regionId)
        }
    }

    private fun handlePokemonClick(ctx: InteractionContext, player: ServerPlayer, regionId: String) {
        val item = ctx.clickedStack
        if (item.item !is PokemonItem) return

        val (species, formName, aspects) =
            parsePokemonName(CustomGui.stripFormatting(item.hoverName.string)) ?: return

        when (ctx.clickType) {
            ClickAction.PRIMARY  -> togglePokemon(player, regionId, species, formName, aspects)
    ...
    private fun togglePokemon(
        ...
        if (existing != null) {
            ...
            player.displayClientMessage(Component.literal("§c[CSR] Removed ${species.name} from region."), false)
        } else {
            try {
                ...
                player.displayClientMessage(Component.literal("§a[CSR] Added ${species.name} to region."), false)
            } catch (e: IllegalArgumentException) {
                RegionsConfig.debugError(logger, "Failed to create default Pokemon entry for $showdownId", e)
                player.displayClientMessage(Component.literal("§c[CSR] ${e.message}"), false)
                return
            }
        }
        refresh(player, regionId)
    ...
    private fun pokemonItem(
        ...
        item.setCustomName(Component.literal((if (isSelected) "§f§n" else "§f") + buildDisplayName(variant, showForms)))
    ...
    fun applySearch(player: ServerPlayer, term: String, regionId: String) {
    ...
    private fun sortBtn(): ItemStack {
        ...
        return CustomGui.createPlayerHeadButton(
            ...
            listOf(
                Component.literal("§7Current: §f${sortMethod.name}"),
                Component.literal("§7Left-click §8to cycle"),
                Component.literal("§7Right-click §8to search")
            ),
            Textures.SORT
        )
    }
    ...
    private fun navBtn(label: String, texture: String) = CustomGui.createPlayerHeadButton(
        ...
        listOf(Component.literal("§7Go to the ${label.lowercase()} page")),
        texture
    )
    ...
    private fun backBtn() = CustomGui.createPlayerHeadButton(
        ...
        listOf(Component.literal("§7Return to Unnatural Spawn settings")),
        Textures.BACK
    )

    private fun defaultsBtn() = ItemStack(Items.WRITABLE_BOOK).apply {
        setCustomName(Component.literal("Default Pokémon Settings").withStyle(ChatFormatting.GOLD))
