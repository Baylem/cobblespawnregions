...
object RegionSpeciesSearchGui {
    ...
    fun open(player: ServerPlayer, regionId: String, target: RestrictionTarget = RestrictionTarget.NATURAL_SPAWNS) {
        ...
        player.displayClientMessage(Component.literal("§7[CSR] §fType a Pokémon name, then click the §agreen button §fto search, or §cX §fto cancel."), false)
    }
    ...
    private fun cancelBtn() = CustomGui.createPlayerHeadButton(
        ...
        title        = Component.literal("§cCancel Search").withStyle { it.withBold(true).withItalic(false) },
        lore         = listOf(Component.literal("§7Return to species list without searching")),
    ...
    private fun dynamicSearchBtn(term: String) = CustomGui.createPlayerHeadButton(
        ...
        lore         = listOf(
            Component.literal("§aClick to search for this term"),
