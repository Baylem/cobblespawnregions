...
object ItemStackSerialization {
    ...
    fun recordThrownBall(entity: Entity) {
        ...
        val ops = RegistryOps.of(JsonOps.INSTANCE, world.server.registryManager)
    ...
    fun stackDebugSummary(stack: ItemStack): String =
        "item=${Registries.ITEM.getId(stack.item)} name='${stack.name.string}' count=${stack.count} empty=${stack.isEmpty}"
    ...
    private fun normalizeCaptureBallJson(raw: String): String {
        val json = JsonHelper.deserialize(raw)
    ...
    private fun entityBallDebugSummary(entity: EmptyPokeBallEntity): String =
        "item=${Registries.ITEM.getId(entity.pokeBall.item())} name='${entity.pokeBall.item().defaultStack.name.string}'"
