...
object RegionSpawnHelper {
    ...
    fun attemptSpawnInRegion(
        ...
        if (region.dimension != world.dimension().location().toString()) return emptyList()
    ...
    fun spawnPokemonAt(
        ...
        val blockAbove   = world.getBlockState(spawnPos.above())
        val blockAbove2  = world.getBlockState(spawnPos.above(2))
        val blockBelow   = world.getBlockState(spawnPos.below())
        RegionsConfig.debugLog(logger, "[CSR-SPAWN] Position $spawnPos diagnostics:")
        RegionsConfig.debugLog(logger, "  block below  (y-1): ${BuiltInRegistries.BLOCK.getKey(blockBelow.block)}")
        RegionsConfig.debugLog(logger, "  block at feet (y+0): ${BuiltInRegistries.BLOCK.getKey(blockAtFeet.block)}")
        RegionsConfig.debugLog(logger, "  block above  (y+1): ${BuiltInRegistries.BLOCK.getKey(blockAbove.block)}")
        RegionsConfig.debugLog(logger, "  block above  (y+2): ${BuiltInRegistries.BLOCK.getKey(blockAbove2.block)}")
        RegionsConfig.debugLog(logger, "  isAir@feet=${blockAtFeet.isAir}  isAir@+1=${blockAbove.isAir}  isAir@+2=${blockAbove2.isAir}")

        if (!blockAtFeet.isAir && !blockAtFeet.`is`(net.minecraft.world.level.block.Blocks.WATER)) {
            RegionsConfig.debugWarn(logger, "[CSR-SPAWN] WARNING: block at feet ($spawnPos) is not air/water: ${BuiltInRegistries.BLOCK.getKey(blockAtFeet.block)}")
        }


        val border = world.worldBorder
        val inBorder = border.isWithinBounds(spawnPos.x.toDouble(), spawnPos.z.toDouble())
        ...
        if (regionId != null && entryKey != null && spawnId != null) {
            ...
            pokemon.persistentData.putLong(RegionEntityTracker.SPAWNED_AT_WORLD_TIME_KEY, world.gameTime)
            pokemon.persistentData.putString(RegionEntityTracker.DIMENSION_KEY, world.dimension().location().toString())
        }

        val spawnX = spawnPos.x + 0.5
        val spawnY = spawnPos.y.toDouble()
        val spawnZ = spawnPos.z + 0.5
        entity.moveTo(spawnX, spawnY, spawnZ, entity.yRot, entity.xRot)
        ...
        return if (world.addFreshEntity(entity)) {
    ...
    fun pickRandomSpawnPos(
        ...
        repeat(minOf(RANDOM_SPAWN_POS_ATTEMPTS, floorCount)) {
            ...
            selected?.let { return BlockPos.of(it) }
        }
        ...
        if (allowedBlocks.isEmpty()) {
            ...
            return selected?.let(BlockPos::of)
        }
    ...
    fun pickClosestSpawnPos(
        ...
        return closest?.let(BlockPos::of)
    }
    ...
    private class SpawnBlockMatcher(
        ...
        private val literalBlockModes = allowedBlocks
            ...
            .mapNotNull { blockId ->
                val id = ResourceLocation.tryParse(blockId.lowercase()) ?: return@mapNotNull null
                val block = BuiltInRegistries.BLOCK.get(id)
                BuiltInRegistries.BLOCK.getId(block) to
    ...
    fun checkBasicSpawnConditions(world: ServerLevel, entry: PokemonSpawnEntry): String? {
        val timeOfDay = world.dayTime % 24000
    ...
    fun isEntryUnderCap(world: ServerLevel, region: RegionData, entry: PokemonSpawnEntry): Boolean {
        if (entry.maxSpawnCount <= 0) return true

        val entryKey = RegionEntityTracker.entryKey(entry)
        val trackedCount = RegionEntityTracker.countForEntry(region.regionId, entryKey)
        val loadedManagedCount = world.getEntitiesOfClass(PokemonEntity::class.java, regionBoundingBox(region)) { entity ->
    ...
    fun regionBoundingBox(region: RegionData): AABB = AABB(
    ...
    fun countPokemonInBox(world: ServerLevel, box: AABB): Int =
        world.getEntitiesOfClass(PokemonEntity::class.java, box) { true }.size





    fun countMatchingInBox(world: ServerLevel, box: AABB, entry: PokemonSpawnEntry): Int {
        ...
        return world.getEntitiesOfClass(PokemonEntity::class.java, box) { entity ->
            val pokemon = entity.pokemon
            if (!pokemon.species.showdownId().equals(targetName, ignoreCase = true)) return@getEntitiesOfClass false
            val liveAspects = pokemon.aspects.map { it.lowercase() }.toSet()
            if (liveAspects != targetAspects) return@getEntitiesOfClass false
            if (!targetForm.isNullOrEmpty()
                ...
                if (!liveForm.equals(targetForm, ignoreCase = true)) return@getEntitiesOfClass false
            }
            true
        }.size
    }
    ...
    private fun isFlyingPosition(world: ServerLevel, pos: BlockPos): Boolean {

        if (!world.getBlockState(pos).isAir) return false
        if (!world.getBlockState(pos.below(1)).isAir) return false


        for (depth in 2..32) {
            val state = world.getBlockState(pos.below(depth))
    ...
    private fun applyCustomIVs(pokemon: Pokemon, entry: PokemonSpawnEntry) {
        ...
        pokemon.setIV(Stats.HP,              random.nextIntBetweenInclusive(iv.minIVHp,             iv.maxIVHp))
        pokemon.setIV(Stats.ATTACK,          random.nextIntBetweenInclusive(iv.minIVAttack,         iv.maxIVAttack))
        pokemon.setIV(Stats.DEFENCE,         random.nextIntBetweenInclusive(iv.minIVDefense,        iv.maxIVDefense))
        pokemon.setIV(Stats.SPECIAL_ATTACK,  random.nextIntBetweenInclusive(iv.minIVSpecialAttack,  iv.maxIVSpecialAttack))
        pokemon.setIV(Stats.SPECIAL_DEFENCE, random.nextIntBetweenInclusive(iv.minIVSpecialDefense, iv.maxIVSpecialDefense))
        pokemon.setIV(Stats.SPEED,           random.nextIntBetweenInclusive(iv.minIVSpeed,          iv.maxIVSpeed))
    }
    ...
    private fun applyHeldItems(pokemon: Pokemon, entry: PokemonSpawnEntry) {
        ...
        heldItems.itemsWithChance.forEach { (itemName, chance) ->
            val id = ResourceLocation.tryParse(itemName) ?: return@forEach
