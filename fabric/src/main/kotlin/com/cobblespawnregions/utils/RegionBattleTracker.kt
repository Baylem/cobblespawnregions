...
class RegionBattleTracker {
    ...
    private fun handlePokemonSent(pokemon: Pokemon) {
        ...
        if (pokemon.entity?.owner is ServerPlayer) {
        ...
        ongoingBattles.forEach { (battleId, battleInfo) ->
            val battleEnded = battleInfo.actors.any { actor ->
                actor is PlayerBattleActor && actor.getPlayerUUIDs().any { uuid ->
                    server.playerList.getPlayer(uuid)?.let { playerIsNotInBattle(it) } ?: true
                }
            }
    ...
    private fun findBattleIdByPlayer(player: ServerPlayer): UUID? =
        ongoingBattles.values.find { battleInfo ->
            battleInfo.actors.any { actor ->
    ...
    private fun playerIsNotInBattle(player: ServerPlayer): Boolean = getActiveBattleAndActor(player) == null

    private fun getActiveBattleAndActor(player: ServerPlayer): Pair<PokemonBattle, BattleActor>? {
